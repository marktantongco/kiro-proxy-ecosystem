...
if ss -tulpn | grep -q ":60000 "; then
    ...
    if ps aux | grep "forward_proxy.py" | grep -qv "systemd" && ps aux | grep "forward_proxy.py" | grep -q "python"; then
...
if ss -tulpn | grep -q ":8333 "; then
    ...
    if [[ -n "$PID" ]]; then
        ...
        if [[ "$PARENT_PID" != "1" && "$PARENT_PID" != "" ]]; then
            warn "Duplicate manual Kiro Gateway detected! (PID: $PID, Parent PPID: $PARENT_PID)"
...
if curl -sf -I -x http://127.0.0.1:60000 https://api.github.com/users/octocat >/dev/null 2>&1; then
